...
function Profile() {
  ...
  const { url, color, profiledata, userData, setuserData, user, follewing, setfollewing, followers, setfollowers, hendletoFollow, profiledataLoading, setprofiledataloading, followersShow, setfollowersShow, followingShow, setfollowingShow } = useContext(StoreContext)
  ...
  return (
    <div className={`px-4 sm:px-0 max-w-[100%] min-w-[100%] sm:max-w-[90%] sm:min-w-[90%] md:max-w-[80%] md:min-w-[80%] lg:max-w-[90%] lg:min-w-[90%] xl:max-w-[80%] xl:min-w-[80%] 2xl:max-w-[70%] 2xl:min-w-[70%] h-full overflow-y-scroll mx-auto ${color === "light" ? "text-black" : "text-white"} relative`}>

      {
        profiledataLoading ? (<div className='flex justify-center items-center mx-auto my-auto'><Spinner /></div>) : (<>
          ...
          <div className='flex w-full min-h-[20vh] pt-4'>
            ...
            <div className='flex flex-col min-w-[70%] max-w-[70%] sm:min-w-[75%] sm:max-w-[75%] items-start justify-center px-4'>
              ...
              <div className='flex gap-4 py-4'>
                ...
                <div className='text-sm sm:text-base text-center font-bold cursor-pointer' onClick={() => setfollowersShow(true)}>{userData.followers?.length} followers</div>
                {
                  followersShow && <div className={`absolute top-24 w-64 md:w-96 right-10 p-4 shadow-xl max-h-[50vh] z-50 overflow-y-scroll rounded-md ${color === "light" ? "bg-white text-black" : "bg-slate-800 text-white"}`}>

                    {
                      userData.followers ? (userData.followers.map((User, index) => {
                        return <div key={index}><span className='flex items-center gap-2 cursor-pointer' onClick={()=> hendleProfile(User._id)}>
                ...
                <div className='text-sm sm:text-base text-center font-bold cursor-pointer' onClick={() => setfollowingShow(true)}>{userData.following?.length} following</div>
                {
                  followingShow && <div className={`absolute top-24 w-64 md:w-96 right-10 p-4 shadow-xl max-h-[50vh] z-50 overflow-y-scroll rounded-md ${color === "light" ? "bg-white text-black" : "bg-slate-800 text-white"}`}>
                    {
                      userData.following ? (userData.following.map((User, index) => {
                        return <div key={index} className='py-2'><span className='flex items-center gap-2 cursor-pointer' onClick={()=> hendleProfile(User._id)}>
                          <span className='text-3xl'>
                            {
                              User.profileImage ? <div className='w-10 h-10 rounded-full border overflow-hidden'><img src={User.profileImage} alt="userImage" className='h-full w-full object-cover object-center' /></div> : <div className='text-5xl'><PiUserCircleThin /></div>
                            }
